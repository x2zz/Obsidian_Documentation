...
function robloxRichTextToHtml(input: string): string {
	...
	let s = String(input);
	s = s.replace(/<\s*b\s*>/gi, () => makePh('<span style="font-weight: bold;">'));
	s = s.replace(/<\s*\/\s*b\s*>/gi, () => makePh("</span>"));
	s = s.replace(/<\s*i\s*>/gi, () => makePh('<span style="font-style: italic;">'));
	s = s.replace(/<\s*\/\s*i\s*>/gi, () => makePh("</span>"));
	s = s.replace(/<\s*u\s*>/gi, () => makePh('<span style="text-decoration: underline;">'));
	s = s.replace(/<\s*\/\s*u\s*>/gi, () => makePh("</span>"));
	s = s.replace(/<\s*s\s*>/gi, () => makePh('<span style="text-decoration: line-through;">'));
	s = s.replace(/<\s*\/\s*s\s*>/gi, () => makePh("</span>"));
...
export default function Label({
	...
	const finalClassName = cn(
		"text-left block text-white text-xs max-w-full min-w-0",
		className,
		doesWrap ? "" : "whitespace-nowrap overflow-hidden"
	);
	...
	if (sanitized !== "") {
		return <span className={finalClassName} style={style} dangerouslySetInnerHTML={{ __html: sanitized }} />;
	}
